...
public class TestMapData {
    ...
    private static final Path GAME_MAP_DIR = Paths.get("src/main/resources/game/bhe/map");
    private static final Path JSON_OUTPUT_DIR = Paths.get("src/main/resources/mapBheJson");
    private static final Path MAP_OUTPUT_DIR = Paths.get("src/main/resources/mapBheGenerated");
    ...
    void testGenerateMapFilesByJson() throws IOException {
        assertMapAssets();
        Assertions.assertTrue(Files.isDirectory(JSON_OUTPUT_DIR), "missing generated bhe map json directory");
        ...
        try (DirectoryStream<Path> stream = Files.newDirectoryStream(JSON_OUTPUT_DIR, "*.json")) {
            for (Path path : stream) {
                ...
                bheBinService.generate(output.toString(), mapData, CHARSET);
    ...
    void testMapDataParseGenerateBinaryConsistency() throws IOException {
        assertMapAssets();
        Assertions.assertTrue(Files.isDirectory(MAP_OUTPUT_DIR), "missing generated bhe map directory");
        ...
        if (!anyIssue && compared > 0) {
            ...
            if (anyIssue) {
                Assertions.fail("Mismatch or missing generated bhe map files detected. Outputs retained for inspection.");
            } else {
                Assertions.fail("No bhe map files compared. Check inputs/outputs before cleanup.");
            }
        }
    }

    private static void assertMapAssets() throws IOException {
        Assertions.assertTrue(Files.isDirectory(GAME_MAP_DIR), "missing bhe map asset directory");
        try (DirectoryStream<Path> stream = Files.newDirectoryStream(GAME_MAP_DIR, "*" + MAP_EXT)) {
            Assertions.assertTrue(stream.iterator().hasNext(), "no bhe map assets found");
        }
    }
}
