...
public class TestBheInfoCollectionDsl {

    private static final Path WAZ_BHE_JSON_DIR = Paths.get("src/main/resources/wazBheJson");
    private static final Path ANALYSIS_RESOURCE_DIR = Paths.get("src/main/resources/bheInfoCollectionAnalysis");
    private static final Path OCCURRENCE_TSV_OUT = ANALYSIS_RESOURCE_DIR.resolve("bhe-info-collection-occurrences.tsv");
    private static final Path PATH_SUMMARY_TSV_OUT = ANALYSIS_RESOURCE_DIR.resolve("bhe-info-collection-path-summary.tsv");
    private static final Path TERM_OVERVIEW_OUT = ANALYSIS_RESOURCE_DIR.resolve("term-overview.tsv");
    private static final Path ANALYSIS_JSON_DIR = Paths.get("src/main/resources/wazBheSemanticJson");

    private final ObjectMapper mapper = new ObjectMapper()
            .configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);

    @Test
    void testAnalyzeBheInfoCollectionDsl() throws IOException {
        Files.createDirectories(ANALYSIS_RESOURCE_DIR);
        Files.createDirectories(ANALYSIS_JSON_DIR);

        TermGrp termGrp = BheInfoCollectionAnalyzer.getCachedTermGrp();
        List<Occurrence> occurrences = new ArrayList<>();

        try (DirectoryStream<Path> stream = Files.newDirectoryStream(WAZ_BHE_JSON_DIR, "*.json")) {
    ...
    private static class Occurrence {
        ...
        private String trailingTypeList;
        private String paramList;
        private String intList3;
        private String intList4;
        private Integer int2;
        private String syntaxPathByName;
        private String syntaxPathByDescription;
        private boolean terminated;
        private String warnings;
        private BheInfoCollectionSemantic semantic;
    }
