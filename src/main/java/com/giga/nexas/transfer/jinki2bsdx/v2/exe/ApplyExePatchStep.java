 ...
 * 容量需求由 {@link BuildExePatchPlanStep} 写入 {@link ExePatchPlan}。</p>
 *
 * <p>每个 patch site 都接受两种合法输入：expected bytes 或 target bytes。
 * 这是链式成果物模型的关键约束：当 BSDX+JINKI 成为下一层 baseline 时，
...
public class ApplyExePatchStep {
     ...
     * V2 保持同类输出习惯，最终 parity 会比较实际生成的 exe bytes。</p>
     */
    ...
    public ExePatchAudit apply(
        ...
        ExePatchProfile patchProfile = profile == null ? ExePatchProfile.defaultAkaoCompatibilityProfile() : profile;
        try {
    ...
    private void applySite(byte[] exeBytes, ExePatchSite site, ExePatchPlan plan, ExePatchAudit audit) {
    ...
    private void applySelectMenuRowPatchIfNeeded(byte[] exeBytes, ExePatchPlan plan, ExePatchAudit audit) {
        ...
        if (targetMaxOffset == 0x33C) {
            // 当前 AKAO 菜单策略是复用第 25 个可见槽位，不新增 SelectMekaMenu 行。
