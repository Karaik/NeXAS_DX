...
public class BheMapAppendSpmDtoConverter {

    public Spm convert(com.giga.nexas.dto.bhe.spm.Spm source) {
        Spm target = new Spm();
        target.setExtensionName("spm");
        if (source == null) {
            target.setSpmVersion("SPM VER-2.00");
            target.setPageData(new ArrayList<>());
            target.setImageData(new ArrayList<>());
            target.setAnimData(new ArrayList<>());
            target.setNumPageData(0);
            target.setNumImageData(0);
            target.setNumAnimData(0);
            target.setPatPageNum(0);
            return target;
        }

        BeanUtil.copyProperties(source, target, "pageData", "imageData", "animData",
                "numPageData", "numImageData", "numAnimData");
        target.setSpmVersion(source.getSpmVersion() == null || source.getSpmVersion().isBlank()
                ? "SPM VER-2.00"
                : source.getSpmVersion());
        target.setPatPageNum(source.getPatPageNum() == null ? 0 : Math.max(0, source.getPatPageNum()));
    ...
    private List<Spm.SPMPatData> convertPats(List<com.giga.nexas.dto.bhe.spm.Spm.SPMPatData> sourceList) {
        ...
        for (com.giga.nexas.dto.bhe.spm.Spm.SPMPatData source : sourceList) {
            ...
            target.setPageNo(source == null || source.getPageNo() == null
                    ? new ArrayList<>()
                    : new ArrayList<>(source.getPageNo()));
            targetList.add(target);
        }
        return targetList;
    }
    ...
    private List<Spm.SPMHitArea> convertHitAreas(List<com.giga.nexas.dto.bhe.spm.Spm.SPMHitArea> sourceList) {
        ...
        for (com.giga.nexas.dto.bhe.spm.Spm.SPMHitArea source : sourceList) {
            targetList.add(normalizeHitArea(source == null ? null : source.transHitbox()));
        }
        return targetList;
    }
    ...
    private Spm.SPMHitArea normalizeHitArea(Spm.SPMHitArea source) {
        ...
        if (target.getHitRect() == null) {
            target.setHitRect(new Spm.SPMRect());
        }
        target.getHitRect().setLeft(valueOrZero(target.getHitRect().getLeft()));
        target.getHitRect().setTop(valueOrZero(target.getHitRect().getTop()));
        target.getHitRect().setRight(valueOrZero(target.getHitRect().getRight()));
        target.getHitRect().setBottom(valueOrZero(target.getHitRect().getBottom()));
