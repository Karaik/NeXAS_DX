 ...
 * 生成两个菜单 SPM 产物。
 *
 * <p>DAT step 只解决“菜单行指向哪个 anim”；本 step 负责让这些 anim 的名称和图片链变成 Tsukuyomi。
 * 它不读取 DAT，也不复制 PNG，只消费已经解析好的 MenuSlotMapping 和 MenuOverrideSpec。</p>
 */
public class PatchMenuSpmStep {
     ...
     * 菜单 SPM 图片链重建器。
     *
     * <p>本 step 只决定“哪个 anim 用哪组图片”，真正的尺寸读取、rect 计算、
     * page/chip/imageData 链重建都交给 rebuilder，方便单独测试布局规则。</p>
     */
    ...
    public String buildSelectMenuAnimName(Mek menuMek) {
        ...
        // 旧 pipeline 当前不在 select-menu animName 末尾加 CR。
        // 这里必须保持一致，否则 SPM bytes 会不同。
