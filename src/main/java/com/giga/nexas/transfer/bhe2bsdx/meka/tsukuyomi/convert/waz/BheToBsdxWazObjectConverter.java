...
/**
 * BHE WAZ 事件对象到 BSDX WAZ 事件对象的源侧结构转换器。
 */
class BheToBsdxWazObjectConverter {
    ...
    SkillInfoObject convert(
        ...
        // BHE slot37 是 CEventFreeParam，BSDX 对应 slot35 的 CEventVal，不能走普通 BeanUtil 复制。
        if (sourceSlot == 37
    ...
    private SkillInfoObject convertFreeParam(
        ...
        for (var unit : sourceFreeParam.getUnitList()) {
            if (unit.getBuffer() == 0) {
                // 真实数据保证存在 buffer==0；该槽 data 可为空，空时沿用 CEventVal 的 0 默认值。
                // 这里不使用 buffer==1 的数据，保持和 src/test 旧 trans 行为一致。
    ...
    private void copyAndTrans(
        ...
        if (target instanceof CEventSpriteAttr event) {
            ...
            // CEventVoice 内部语音组目标重绑后置，本阶段保持源侧 byteDataList。
            BeanUtil.copyProperties(source, event);
