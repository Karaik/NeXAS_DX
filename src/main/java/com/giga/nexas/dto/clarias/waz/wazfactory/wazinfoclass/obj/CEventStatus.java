...
public class CEventStatus extends SkillInfoObject {
    ...
    public static final CEventStatusType[] CEVENT_STATUS_TYPES = {
            ...
            new CEventStatusType(0x7, "エフェクト") //diff
    };
    ...
    public void readInfo(BinaryReader reader) {
        ...
        for (int i = 0; i < 25; i++) {
            ...
            if (buffer != 0) {
                int innerTypeId = CEVENT_STATUS_TYPES[i].getType();
                if (innerTypeId == 0xFFFFFFFF) {
                    throw new OperationException(500, "unexpected non-zero status wrapper buffer at slot " + i);
                }
                SkillInfoObject obj = createCEventObjectByTypeClarias(innerTypeId);
                if (obj == null) {
                    throw new OperationException(500, "missing status wrapper type at slot " + i + ": " + innerTypeId);
                }
                obj.readInfo(reader);
                unit.setData(obj);
