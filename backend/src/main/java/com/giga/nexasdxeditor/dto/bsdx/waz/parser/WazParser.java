...
//                    throw new BusinessException(500, "文件数据格式错误！（起始符）");
                ...
                // 3.技能数据
                offset = parseWazPhaseInfo(waz, bytes, offset);
...
//    }
    ...
    private static int parseWazPhaseInfo(Waz waz, byte[] bytes, int offsetInner) {
    ...
    private static int setWazPhaseUnit(Waz.WazPhase wazPhase, byte[] bytes, int offset) {
        // 存放单个阶段的数据信息
        List<List<WazInfoObject>> wazInfoObjectCollection = wazPhase.getWazInfoObjectCollection();

        for (int i = 0; i < 72; i++) { // 逆向得知循环72次
            List<WazInfoObject> wazInfoObjects = new ArrayList<>();
            ...
            for (int j = 0; j < count; j++) {
                ...
                wazInfoObjects.add(eventObject);
            }

            wazInfoObjectCollection.add(wazInfoObjects);
        }
